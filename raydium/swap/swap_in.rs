...
use crate::{
    app::{amount_percentage, priority_fee, sol_amount, token_env},
    env::load_settings,
...
use super::{
    raydium_swap_in::{raydium_in,bundle_transactions},
...
pub async fn swap_in() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let token_out = token_env().await?;

    let private_key =
        Keypair::from_bytes(&bs58::decode(&args.payer_keypair[0]).into_vec().unwrap())?;
    let rpc_client = RpcClient::new(args.rpc_url.to_string());
    println!("{:?}",args.rpc_url.to_string());
    let pool_keys = match pool_keys_fetcher(token_out).await {
        Ok(keys) => keys,
        Err(e) => {
            // Handle the error, possibly by logging it
            error!("Error fetching pool keys: {:?}", e);
            return Err(e.into()); // Convert the error into a Box<dyn std::error::Error>
        }
    };    println!("{:?}",pool_keys);
...
pub async fn swap_in_bundle() -> Result<(), Box<dyn std::error::Error>> {
    let args = match load_settings().await {
        Ok(args) => args,
        Err(e) => {
            error!("Error: {:?}", e);
